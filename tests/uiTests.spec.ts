...
test("check checkout btn", async ({ page }) => {
    await page.locator(`//*[@data-test='inventory-item-name' and contains(text(), 'Bolt T-Shirt')]`).click;
...
test('check amount of added items in cart', async ({ page }) => {
    ...
    const itemAmount = await page.locator('[data-test="shopping-cart-badge"]').textContent()
...
test('do items added to cart', async ({ page }) => {
    ...
    await expect(page.locator('//div[@class="cart_item_label"][.//div[@data-test="inventory-item-name" and text()="Sauce Labs Bolt T-Shirt"]] //button[starts-with(@data-test,"remove-")]'))
    .toBeVisible
})
...
test('check error for checkout without last name', async ({ page }) => {
    ...
    await page.locator('[data-test="continue"]').click()
    await expect(page.locator('//h3[@data-test="error" and text()="Error: Last Name is required"]')).toBeVisible
})
