...
import { validateErrorResponse } from "./api-controller";

const baseUrl = "https://pu5hds6usi.execute-api.us-east-1.amazonaws.com/mocks";

test.describe("Mocks page", () => {
  test("should display data on successful respnse", async ({ page }) => {
  ...
  test("should display expected loading state on slow response", async ({
    page,
  }) => {
    await page.route(baseUrl + "?action=getData", (route) => {
  ...
  validateErrorResponse(baseUrl, 418, "Test message for 418");
